/*
    ...
    ON MAC USE ESP8266 BOARD VERSION 3.02               (DO NOT UPDATE!) (TS 3-27-24)
            Newer board driver versions do not complie on MacOS due to Python3 version incompatability

    REMOVED ALL COMPILE MACRO REFERENCES / USE OF TALLY_TEST_SERVER CODE (TS 3-27-24)

    ADDED   DEFAULT TALLY NUMBER IF NOT DEFINED IN RANGE                 (TS 3-28-24)

    ADDED   Live Source Mode - defined by list of ATEM sources that together with the current streaming status devine "On Air Live" 
            this is used to control the relay shield in order to provide an "On Air" light that is only active when one of the identified 
            sources is in Program and the ATEM is streaming. This functionality allows for some sources to be active without activating the 
            relay (Light) therefor allowing for a media presented as a "starting soon screen" or pre-recorded media source. 
            This functionality provides a simple and easily understood single source indicator for those presenting that they are 
            currently the focus of the stream (Live On Air). 
            
            The Setup Web page now has a Live Sources text entry box. Here all of the sources that should be considered 'Live' as described
            above can be entered as a single line of text. For example if any of the sources 1, 2 or 5 actie on the Program bus along with
            streaming On Air to activate the Relay enter "1,2,5" in the Live Sources Text field. This is the pre-defined example.
            
            The Live Sources field will support 30 characters and any sources 1-40 can be included.                             (TS 3-28-24)
    
    ADDED   Added mode test to 'Connecting to WiFi section (about 348-364) to avoid a false condition turning on the on air sign during
            boot up and connection initialization. This can be an issue for devices that are powered all the time and the ATEM Switcher
            is not. See about line 362 below: if (settings.tallyModeLED1 == MODE_ON_AIR_LIVE) setBothLEDs(LED_OFF); else setBothLEDs(LED_WHITE);
    
    NOTES   This modified version can be compiled and used with the LED's as originally desribed in the DIY Guide
            The Red & Blue connections for LED1 are reversed in order to also support the Relay Shield if used. 
...
#define DISPLAY_NAME "Tally & On Air Light"
...
#ifndef PIN_RED1
#define PIN_RED1 32  //Set to D1 pin for Relay Shield
#endif
#ifndef PIN_GREEN1
...
#ifndef PIN_RED2
#define PIN_RED2 26  //Set to D1 pin for Relay Shield
#endif
#ifndef PIN_GREEN2
...
//Define LED1 color pins
#ifndef PIN_RED1
#define PIN_RED1 5  // Set to D1 pin for Relay Shield (Red & Blue pins are swapped from original DIY guide instructions)
#endif
#ifndef PIN_GREEN1
#define PIN_GREEN1 4  // D2
#endif
#ifndef PIN_BLUE1
#define PIN_BLUE1 16  // Set to D0 pin for Relay Shield
#endif

//Define LED2 color pins
#ifndef PIN_RED2
#define PIN_RED2 2  // D4 - This is also the D1 Wemos on-board LED
#endif
#ifndef PIN_GREEN2
...
#define MODE_ON_AIR_LIVE 5  //ADDED to test both Streaming & Specific Tally Conditions (Sources 1,2,5) for Live Activity
#define MODE_OFF 6          //ADDED to disable LED #2 when not being used at all. 
...
struct Settings {
  char tallyName[32] = "";
  char liveSources[32] = "";    //ADDED to test both Streaming & Specific Tally Conditions (Default Sources 1,2,5) for Live Activity
  uint8_t tallyNo;
...
void loop() {
  ...
  switch (state) {
    case STATE_CONNECTING_TO_WIFI:
      if (WiFi.status() == WL_CONNECTED) {
        ...
        firstRun = false;
        Serial.println("Unable to connect. Serving \"Tally & On Air Light Setup\" WiFi for configuration, while still trying to connect...");
        WiFi.softAP((String)DISPLAY_NAME + " Setup");
        WiFi.mode(WIFI_AP_STA);  // Enable softAP to access web interface in case of no WiFi
        if (settings.tallyModeLED1 == MODE_ON_AIR_LIVE) setBothLEDs(LED_OFF); else setBothLEDs(LED_WHITE);
        //setBothLEDs(LED_WHITE);  // The statement above avoides a false indicator on the On Air Sign Relay while waiting to connect to the switcher. 
  ...
  if (WiFi.status() != WL_CONNECTED && state != STATE_CONNECTING_TO_WIFI) {
    ...
    //Reset tally server's tally flags, They won't get the message, but it'll be reset for when the connection is back.
    tallyServer.resetTallyFlags();
  }
...
void changeState(uint8_t stateToChangeTo) {
  ...
  switch (stateToChangeTo) {
    ...
    case STATE_CONNECTING_TO_SWITCHER:
      ...
      if (settings.tallyModeLED1 == MODE_ON_AIR_LIVE) setBothLEDs(LED_OFF); else setBothLEDs(LED_PINK);
      //setBothLEDs(LED_PINK); // The statement above avoides a false indicator on the On Air Sign Relay while waiting to connect to the switcher. 
...
int getLedColor(int tallyMode, int tallyNo) {
  ...
  String camSources = String(settings.liveSources);

  if (tallyMode == MODE_OFF) {
    return LED_OFF;
  }
  if (tallyMode == MODE_ON_AIR) { //Streaming Indicator
  ...
  if (tallyMode == MODE_ON_AIR_LIVE) {
    //TEST LIVE SOURCES FOR PROGRAM STATE - Streaming AND using any of a list of sources

    if (atemSwitcher.getStreamStreaming()) {
       for (uint8_t i = 0; i < 40; i++) {
        // Test valid and available sources
        if (i >= atemSwitcher.getTallyByIndexSources()) {  //out of range
          i = 41;                                           // all valid sources checked exit loop
        } // end valid source check

        // test sources for program state
        if ((TALLY_FLAG_PROGRAM == getTallyState(i)) && (i < 41)) {
          // test if program source is source in Live List
          if (camSources.indexOf(String(i+1))>-1) { // Is this a listed 'Live' source?
              return LED_RED; // Turn on light (relay) and exit function call
          }  // End Live Source check
        } // End Program Flag Check
      } // end_for loop through available sources
       return LED_OFF; // turn off light (relay) and exit function call, no valid live source found 
    } else { // not streaming
      return LED_OFF; // turn off light (relay) and exit function call, not streaming
    } //endif streaming
    
  } // end of MODE_ON_AIR_LIVE
...
void handleRoot() {
  ...
  html += "<tr bgcolor=\"#305701\"style=\"color:#ffffff;font-size:.8em;\"><td colspan=\"3\"><h2>&nbsp;Settings:</h2></td></tr><tr><td><br></td></tr><form action=\"/save\"method=\"post\"><tr><td>Tally Light name: </td><td><input type=\"text\"size=\"30\"maxlength=\"30\"name=\"tName\"value=\"";
#if ESP32
...
if (settings.tallyNo > 40) {
  ...
  if (settings.tallyModeLED1 == MODE_ON_AIR)
    html += "selected";
  html += ">On Air Streaming</option><option value=\"";
  html += (String)MODE_ON_AIR_LIVE + "\"";
  if (settings.tallyModeLED1 == MODE_ON_AIR_LIVE)
    html += "selected";
  html += ">On Air Light</option></select></td></tr><tr><td>Tally Light mode (LED 2):</td><td><select name=\"tModeLED2\"><option value=\"";
  html += (String)MODE_NORMAL + "\"";
  ...
  if (settings.tallyModeLED2 == MODE_ON_AIR)
    html += "selected";
  html += ">On Air Streaming</option><option value=\"";
  html += (String)MODE_OFF + "\"";
  if (settings.tallyModeLED2 == MODE_OFF)
    html += "selected";
  html += ">LED Off</option></select></td></tr>";
   
  // Gather list of On Air Live Sources that one of which must be in Program mode while streaming to set relay shield on
  if (String(settings.liveSources).length() > 30) { //If undefined - set to default suggestion
    strncpy(settings.liveSources,"1,2,5",32);
  }
  html += "<tr><td><br></td></tr>"; // Blank line - Spacing
  html += "<tr><td> Tally On Air Live Sources (e.g. 1,2,5): </td><td><input type=\"text\" size=\"30\" maxlength=\"30\" name=\"tLiveSources\" value=\"";
  html += settings.liveSources;
  html += "\"required/></td></tr>"; 
  html += "<tr><td><br></td></tr>"; // Blank line - Spacing
  //

  // html += "<tr><td><br></td></tr>"; // Blank line - Spacing
  html += "<tr><td> Led brightness: </td><td><input type=\"number\"size=\"5\"min=\"0\"max=\"255\"name=\"ledBright\"value=\"";
  ...
  html += "<tr><td>Note: On Air Streaming and<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp On Air Light modes<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp require a direct connection<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp to the switcher<br>";

  html += "<tr><td></td></tr><tr><td/><td style=\"float: right;\"><input type=\"submit\"value=\"Save Changes\"/></td></tr></form><tr><td><br></td></tr><tr bgcolor=\"#b8ed77\"style=\"font-size: .8em;\"><td colspan=\"3\"><p>&nbsp;&copy; 2020-2022 <a href=\"https://aronhetlam.github.io/\">Aron N. Het Lam</a>, with Relay Shield and Live Source modifications by Tim Smith &nbsp;&copy; 2024</p><p>&nbsp;Based on ATEM libraries for Arduino by <a href=\"https://www.skaarhoj.com/\">SKAARHOJ</a></p></td></tr></table></body></html>";
...
void handleSave() {
  if (server.method() != HTTP_POST) {
    server.send(405, "text/html", "<!DOCTYPE html><html><head><meta charset=\"ASCII\"><meta name=\"viewport\"content=\"width=device-width, initial-scale=1.0\"><title>Tally & On Air Light Setup</title></head><body style=\"font-family:Verdana;\"><table bgcolor=\"#305701\"border=\"0\"width=\"100%\"cellpadding=\"1\"style=\"color:#ffffff;font-size:.8em;\"><tr><td><h1>&nbsp;" + (String)DISPLAY_NAME + " setup</h1></td></tr></table><br>Request without posting settings not allowed</body></html>");
  } else {
    String ssid;
    String pwd;
    String sources;
    ...
    for (uint8_t i = 0; i < server.args(); i++) {
      ...
      //Serial.println(var+" / "+val); //Used to display elements of the settings structure for debugging
      if (var == "tName") {
        val.toCharArray(settings.tallyName, (uint8_t)32);
      } else if (var == "tLiveSources") {
        val.toCharArray(settings.liveSources, (uint8_t)32);
    ...
    if (change) {
      ...
      server.send(200, "text/html", (String) "<!DOCTYPE html><html><head><meta charset=\"ASCII\"><meta name=\"viewport\"content=\"width=device-width, initial-scale=1.0\"><title>Tally & On Air Light Setup</title></head><body><table bgcolor=\"##305701\"border=\"0\"width=\"100%\"cellpadding=\"1\"style=\"font-family:Verdana;color:#ffffff;font-size:.8em;\"><tr><td><h1>&nbsp;" + (String)DISPLAY_NAME + " Setup</h1></td></tr></table><br>Settings saved successfully.</body></html>");
...
void handleNotFound() {
  server.send(404, "text/html", "<!DOCTYPE html><html><head><meta charset=\"ASCII\"><meta name=\"viewport\"content=\"width=device-width, initial-scale=1.0\"><title>" + (String)DISPLAY_NAME + " Setup</title></head><body style=\"font-family:Verdana;\"><table bgcolor=\"#305701\"border=\"0\"width=\"100%\"cellpadding=\"1\"style=\"color:#ffffff;font-size:.8em;\"><tr><td><h1>&nbsp Tally & On Air Light Setup</h1></td></tr></table><br>404 - Page not found</body></html>");
}
